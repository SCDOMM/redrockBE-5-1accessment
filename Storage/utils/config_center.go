...
var (
	...
	RegistryConfigSample *RegistryConfig
)
...
func init() {
	configOnce.Do(func() {
		...
		if err != nil {
			log.Fatalf("读取配置失败！" + err.Error())
		}
		err = yaml.Unmarshal(dataBytes, &RegistryConfigSample)
		if err != nil {
			log.Fatalf("解析配置失败！" + err.Error())
		}
...
func GetEtcdEndpoints() []string {
	return []string{fmt.Sprintf("%s:%d", EtcdConfigSample.Host, EtcdConfigSample.Port)}
}
